...
#[AsCommand(
    name: 'app:delete-old-rejected-advert',
    description: 'Delete old rejected Adverts.',
    aliases: ['app:del-old-rej-adv'],
    hidden: false
)]
class deleteOldRejectedAdvert extends Command
{
    ...
    {
        $this
            ->setDescription('Delete old rejected Advert')
            ->setHelp('This command allows you to delete old rejected Advert by day.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (ctype_digit($days)) {
            ...
            $oldAdverts = $this->advertRepository->getOldAdvert($date);
            if ($oldAdverts) {
                $deletedCount = 0;
